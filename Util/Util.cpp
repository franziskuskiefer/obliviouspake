...
gcry_mpi_t* Util::MessageToS(Botan::OctetString in, int numPwds){
	gcry_mpi_t *S;
...
Botan::BigInt Util::ihmeDecode(message m, Botan::DL_Group G, int c, Botan::BigInt pwd){
	gcry_mpi_t p;
	...
	BigIntToMpi(&serverPwdNumMPI, pwd);
...
void Util::addElement(struct point *P, int *pos, Botan::BigInt pwd, Botan::BigInt m){
...
void Util::addOctetString(Botan::OctetString toAdd, std::vector<Botan::byte> *vec) {
...
Botan::OctetString Util::encodeS(gcry_mpi_t *S, int c){
...
void Util::keyGen(Botan::OctetString K, Botan::OctetString *finalK, Botan::InitializationVector *iv, std::vector<Botan::byte> sid){
	std::string forKey = "1";
...
void Util::confGen(Botan::OctetString K, Botan::OctetString *conf, Botan::InitializationVector *iv, std::vector<Botan::byte> sid){
...
Botan::SecureVector<Botan::byte> Util::PRF(Botan::OctetString k, Botan::SecureVector<Botan::byte> sid, std::string indicator, Botan::InitializationVector *iv){
...
gcry_mpi_t* Util::createIHMEResultSet(int numPwds){
	gcry_mpi_t *S;
	S = (gcry_mpi_t*)calloc(numPwds, sizeof(gcry_mpi_t));
	for(int k = 0; k < numPwds; k++)
		S[k] = gcry_mpi_new(0);
